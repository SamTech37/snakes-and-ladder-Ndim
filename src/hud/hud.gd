...
func _fight(f: Dictionary, s: Dictionary) -> String:
	...
	# The round that just went, and whatever the tray was asked about, both on the line below. Up here it pushed the prompt off the edge of the screen.
	var below := PackedStringArray()
	if not f["last"].is_empty():
		below.append(f["last"])
	if not s["notice"].is_empty():
		below.append(s["notice"])
	...
	if f["game"] < 0:
		...
		return "%s%s  -  UP/DOWN, SPACE:  %s%s" % [head, matchup, "  ".join(parts), tail]
	return "%s%s under %s  -  SPACE stakes it%s" \
			% [head, matchup, _hi(MG.ALL[f["game"]].label(), Pal.C_FOE), tail]
