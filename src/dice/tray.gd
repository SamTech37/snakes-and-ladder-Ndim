...
func _ready() -> void:
	# One spare is authored; the rest are copies of it. Five hand-placed boxes in the
...
func set_kit(kit: PackedInt32Array, sel: int) -> void:
	...
	for i in dice.size():
		...
		var c: Color = Pal.C_MOVE if on else Pal.C_MOVE * 0.4
		d.material_override = _mat(c)
		var face: Label3D = d.get_node("Face")
		face.text = "d%d" % kit[i]
		# Ink on the lit face, the face's own color on the dim one -- either way the
		# number reads against what is behind it.
		face.modulate = Pal.C_DIE_INK if on else Pal.C_MOVE
